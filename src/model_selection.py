...
#     Returns:
#     - results_df: pandas DataFrame containing the performance of all models
#     """
    
#     # 1. Split data into features (X) and target (y)
# #   df = df.columns.str.replace('<', '', regex=True).str.replace('>', '', regex=True).str.replace(' ', '_', regex=True)
...
# # Example usage:
# # csv_path = 's3://sagemaker-ap-south-1-021891590414/housing.csv'
# # df = pd.read_csv(csv_path)
# # df = pd.read_csv('your_data.csv')  # Replace with your dataset
# # results = train_test_model_selection(df, target_column='your_target_column', task='classification')
# # results = train_test_model_selection(df, target_column='median_house_value', task='regression')
